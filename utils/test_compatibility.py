...
class TestRunner:
    ...
    def download_linksocks(self) -> Path:
        ...
        if local_linksocks.exists() and local_linksocks.is_file():
            ...
            if os.access(local_linksocks, os.X_OK):
                ...
                print(f"Local linksocks file exists but is not executable: {local_linksocks}")
        ...
        if local_linksocks_exe.exists() and local_linksocks_exe.is_file():
            if os.access(local_linksocks_exe, os.X_OK):
                print(f"Using local linksocks executable: {local_linksocks_exe.absolute()}")
        ...
        if not asset_url:
            ...
            for asset in release_info['assets']:
                print(f"  - {asset['name']}")
            raise ValueError(f"Could not find suitable download file for current system: {asset_name}")
    ...
    def check_linksocks_help(self):
        ...
        try:
            ...
            # Check server help
            result = subprocess.run([str(self.linksocks_path), "server", "--help"], 
            ...
            # Check client help  
            result = subprocess.run([str(self.linksocks_path), "client", "--help"], 
                                  capture_output=True, text=True, timeout=10)
            if result.returncode == 0:
    ...
    def start_pywssocks_server(self, port: int = None, reverse: bool = False) -> Tuple[subprocess.Popen, int, int]:
        ...
        cmd = [
            sys.executable, "-m", "pywssocks", "server",
            "-t", TEST_TOKEN,
            "--ws-host", "127.0.0.1",
            "--ws-port", str(port),
            "-dd",
    ...
    def start_pywssocks_client(self, ws_url: str, socks_port: int = None, reverse: bool = False) -> Tuple[subprocess.Popen, int]:
        ...
        cmd = [
            sys.executable, "-m", "pywssocks", "client",
            "-t", TEST_TOKEN,
            "--url", ws_url,
            "--socks-port", str(socks_port),
            "-dd",
        ...
        if not reverse:
            ...
            if not self.wait_for_port(socks_port, 10):
                ...
                try:
                    ...
                    proc.kill()
                raise RuntimeError(f"pywssocks client failed to start on SOCKS port {socks_port}")
                
        return proc, socks_port
        
    def start_linksocks_server(self, port: int = None, socks_port: int = None, reverse: bool = False) -> Tuple[subprocess.Popen, int, int]:
        ...
        cmd = [
            str(self.linksocks_path), "server",
            "-t", TEST_TOKEN,
            "-H", "127.0.0.1",
            "-P", str(port),
            "-dd",
    ...
    def start_linksocks_client(self, ws_url: str, socks_port: int = None, reverse: bool = False) -> Tuple[subprocess.Popen, int]:
        ...
        cmd = [
            str(self.linksocks_path), "client",
            "-t", TEST_TOKEN,
            "-u", ws_url,
            "--no-env-proxy",
            "-dd",
        ...
        if not reverse:
            ...
            if not self.wait_for_port(socks_port, 10):
                ...
                try:
                    ...
                    proc.kill()
                raise RuntimeError(f"linksocks client failed to start on SOCKS port {socks_port}")
    ...
    def test_reverse_proxy_linksocks_to_pywssocks(self) -> bool:
        ...
        try:
            ...
            ws_url = f"ws://127.0.0.1:{ws_port}/socket"
            client_proc, _ = self.start_pywssocks_client(ws_url, reverse=True)  # reverse mode doesn't need port
            print("   Reverse client started")
            time.sleep(3)
    ...
    def run_all_tests(self) -> bool:
        ...
        try:
            ...
            print("="*60)
            print(f"Test 1 - Forward (pywssocks→linksocks): {'✅ Pass' if test1_result else '❌ Fail'}")
            print(f"Test 2 - Forward (linksocks→pywssocks): {'✅ Pass' if test2_result else '❌ Fail'}")
            print(f"Test 3 - Reverse (pywssocks→linksocks): {'✅ Pass' if test3_result else '❌ Fail'}")
            print(f"Test 4 - Reverse (linksocks→pywssocks): {'✅ Pass' if test4_result else '❌ Fail'}")
            ...
            if "rate limit" in str(e).lower() or "could not get release" in str(e).lower():
...
def main():
    ...
    try:
        ...
        with TestRunner() as runner:
            ...
            print("2. If download failed, try again later or manually download linksocks")
